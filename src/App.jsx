...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (email) {
      let u = DB.getUser(email)
      if (u) {
        // Check if Pro trial expired → downgrade to free
        u = checkTrialExpiry(u)
        DB.saveUser(email, u)
        setUser(u)
        setLeads(DB.getLeads(email))
        setClients(DB.getClients(email))
  ...
  useEffect(() => {
    document.title = PAGE_TITLES[tab] || "Zenvoy — AI Lead Scanner"
  }, [tab, PAGE_TITLES])
  ...
  const auth = useCallback(u => {
    setUser(u); setLeads(DB.getLeads(u.email)); setClients(DB.getClients(u.email))
    setAuthMode(null); if (!u.onboarded) setShowBoard(true)
  }, [])

  const logout = useCallback(() => {
    DB.clearSession(); setUser(null); setLeads([]); setClients([]); setTab("home")
  }, [])

  const doneBoard = useCallback(() => {
    const u = { ...user, onboarded: true }; DB.saveUser(user.email, u); setUser(u); setShowBoard(false)
  }, [user])

  const handleLeads = useCallback((newLeads, updUser) => {
    setLeads(prev => { const merged = [...newLeads, ...prev]; DB.saveLeads(updUser.email, merged); return merged })
    setUser(updUser); setShowSearch(false); showToast("Found " + newLeads.length + " new leads!"); setTab("leads")
  }, [showToast])

  const updLead   = useCallback(l  => { setLeads(prev => { const n = prev.map(x => x.id === l.id ? l : x);   DB.saveLeads(user?.email, n); return n }) }, [user])
  const delLead   = useCallback(id => { setLeads(prev => { const n = prev.filter(l => l.id !== id);           DB.saveLeads(user?.email, n); return n }); showToast("Lead deleted.") }, [user, showToast])
  const addClient = useCallback(c  => { setClients(prev => { const n = [c, ...prev];                          DB.saveClients(user?.email, n); return n }); showToast("Client added!") }, [user, showToast])
  const updClient = useCallback(c  => { setClients(prev => { const n = prev.map(x => x.id === c.id ? c : x); DB.saveClients(user?.email, n); return n }); showToast("Saved!") }, [user, showToast])
  const delClient = useCallback(id => { setClients(prev => { const n = prev.filter(c => c.id !== id);         DB.saveClients(user?.email, n); return n }); showToast("Removed.") }, [user, showToast])
  const handleUpgrade = useCallback((feature, plan) => setUpgradeFor({ feature, plan }), [])
  const openSearch = useCallback(() => setShowSearch(true), [])

  // Compute scan info — free plan uses scanQuota, paid plans use user.scansUsed
  const scansLeft = useMemo(() => {
    if (!user) return 0
    if (user.plan === "free") return getFreeScansLeft()
    return getScansLeft(user)
  }, [user, tab]) // re-check when tab changes (after scan)
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="app-sidebar">
        ...
        <div className="sidebar-scan-pill" style={{ marginBottom: 11, padding: "8px 11px", borderRadius: 9,
          ...
          <div style={{ height: 3, borderRadius: 2, background: "var(--s3)", overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 2, transition: "width .4s", background: scansLeft === 0 ? "var(--red)" : scansLeft <= 2 ? "var(--amber)" : "var(--lime)",
              width: ((scansLeft / (user.plan === "free" ? 3 : getScansLimit(user))) * 100) + "%" }} />
          </div>
          <div className="scan-detail" style={{ fontSize: 10, color: "var(--txt3)", marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{leadsN} leads/scan · {plan.name}</div>
        </div>

        <button className="btn btn-lime sidebar-search-btn" style={{ width: "100%", justifyContent: "center", marginBottom: 12, fontSize: 13, padding: "10px", position: "relative", boxShadow: scansLeft > 0 ? "0 0 14px rgba(198,241,53,.3)" : "none" }} onClick={openSearch} aria-label="New Search">
          <I n="search" s={14} /><span>New Search</span><span style={{ marginLeft: "auto", fontSize: 10, opacity: .55, fontWeight: 400 }}>⌘K</span>
        </button>

        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflow: "hidden" }}>
          {NAV.map(item => (
            <button key={item.id} className={"nav-btn" + (tab === item.id ? " on" : "")} onClick={() => setTab(item.id)} style={{ overflow: "hidden" }} aria-label={item.label} aria-current={tab === item.id ? "page" : undefined}>
        ...
        <div style={{ borderTop: "1.5px solid var(--brd)", paddingTop: 10, marginTop: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px", marginBottom: 5 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="plan-name" style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".04em", color: plan.color || "var(--txt3)" }}>{plan.name}{trialActive ? " (trial)" : ""}</div>
            </div>
          </div>
      ...
      <main className="app-main">
        ...
        {tab === "leads"        && <LeadsPage user={user} leads={leads} onUpdate={updLead} onDelete={delLead} onSearch={openSearch} onUpgrade={handleUpgrade} />}
        {tab === "clients"      && <ClientsPage clients={clients} leads={leads} onAdd={addClient} onUpdate={updClient} onDelete={delClient} />}
        {tab === "community"    && <CommunityPage user={user} onGoSettings={() => setTab("settings")} />}
        {tab === "tools"        && <ToolsPage user={user} onUpgrade={handleUpgrade} />}
        {tab === "analytics"    && <AnalyticsPage leads={leads} clients={clients} />}
        {tab === "history"      && <HistoryPage user={user} />}
        {tab === "subscription" && <SubscriptionPage user={user} onUpdate={u => { setUser(u); DB.saveUser(u.email, u) }} />}
        {tab === "settings"     && <SettingsPage user={user} onUpdate={u => { setUser(u); DB.saveUser(u.email, u) }} onLogout={logout} onGoSubscription={() => setTab("subscription")} />}
      ...
      {/* Mobile FAB */}
      <button className="mob-fab" onClick={openSearch} aria-label="New scan">
        <I n="search" s={16} c="#0c0e13" />Scan
      </button>
      ...
      <nav className="bottom-nav" aria-label="Mobile navigation">
        {[
          ...
          <button key={item.id} className={"bnav-btn" + (tab === item.id ? " on" : "")} onClick={() => setTab(item.id)} aria-label={item.label} aria-current={tab === item.id ? "page" : undefined}>
            <I n={item.icon} s={item.id === tab ? 20 : 18} c={tab === item.id ? "var(--lime)" : "var(--txt3)"} />
            <span>{item.label}</span>
          </button>
        ))}
        <button className={"bnav-btn" + (moreActive ? " on" : "")} onClick={() => setShowMore(true)} aria-label="More options">
          <I n="menu" s={18} c={moreActive ? "var(--lime)" : "var(--txt3)"} />
          <span>More</span>
        </button>
      </nav>
          ...
          <div className="mob-drawer" onClick={e => e.stopPropagation()}>
              ...
              {[
                ...
                <button key={item.id}
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px",
                    background: tab === item.id ? "rgba(198,241,53,.07)" : "var(--s2)",
                    border: "1.5px solid " + (tab === item.id ? "var(--lime)" : "var(--brd)"),
            ...
            <div style={{ borderTop: "1.5px solid var(--brd)", paddingTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: plan.best ? "var(--lime)" : "var(--s3)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--fh)", fontWeight: 900, fontSize: 14, color: plan.best ? "#0c0e13" : "var(--txt)" }}>
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{user.name}</div>
                  <div style={{ fontSize: 11, color: plan.color || "var(--txt3)", fontWeight: 700, textTransform: "uppercase" }}>{plan.name} Plan</div>
                </div>
              </div>
              <button className="btn btn-ghost" style={{ fontSize: 12, padding: "8px 14px" }} onClick={logout}>
